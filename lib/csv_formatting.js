...
function csvAssignmentsLines(assignments, startTime) {
	return assignments
		.reduce((lines, assignment) => {
			...
			if (line) {
				...
				lines.push({ start: assignment.slot.start, end: assignment.slot.end, teams: [assignment.team.number] })
			}
			return lines
		}, [])
...
function roomBlock({ roomType, assignments, roomsCount }, startTime) {
	const lines = csvAssignmentsLines(assignments, startTime)
	const header = [
...
const FILE_PART_FUNCTIONS = [
	...
	function rankings(scheduele) {
		const lines = csvAssignmentsLines(scheduele.rankingMatches(), scheduele.startTime())
		const header = [
	...
	function judgesRooms(scheduele) {
		...
		return block(3, header.concat(roomsAssignments.reduce((lines, roomAssignments) =>
			lines.concat(roomBlock(roomAssignments, scheduele.startTime())), [])))
	},
	function practice(scheduele) {
		const lines = csvAssignmentsLines(scheduele.practiceMatches(), scheduele.startTime())
		const header = [
