...
exports.Population = class Population {
	...
	calculateFitness () {
		...
		console.log(`#${this.gen}			alpha: ${this.formattedGrade(this.alpha.fitness)}			omega: ${this.formattedGrade(this.omega.fitness)}`)
	}

	formattedGrade (grade) {
		const percision = 3
		return Math.floor((grade / this.desiredFitness * 100) * Math.pow(10, percision)) / Math.pow(10, percision)
	}
