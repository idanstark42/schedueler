...
class ScheduelingOption {
	...
	generateDNA () {
		...
		const availableSlots = this.emptyScheduele.slots.slice()
		...
		while (unassignedSessions.length > 0) {
			...
			// Find optional slots for this session
			const optionalSlots = availableSlots.filter(slot => {
			...
			if(optionalSlots.length === 0) {
				// Removing all sessions that are of the same type
				let indeces = []
				this.dna.forEach((slot, sessionIndexA) => {
					if(slot && slot.type === session.session.requiredSlotType) {
						indeces.push(sessionIndexA)
					}
				})
				indeces.forEach(index => {
					availableSlots.push(this.dna[index])
					this.dna[index] = null
					unassignedSessions.push(this.emptyScheduele.sessions[index])
					teamUnavailabilities[this.emptyScheduele.sessions[index].team.number] = teamUnavailabilities[this.emptyScheduele.sessions[index].team.number]
						.filter(unavilability => unavilability.sessionIndex !== index)
				})
				// Removing all team sessions
				teamUnavailabilities[session.team.number].forEach(unavilability => {
					availableSlots.push(this.dna[unavilability.sessionIndex])
			...
			// Select a random slot from the optional ones for this session and remove it from the list of available slots
	...
	roomsAssignments () {
		return this.emptyScheduele.roomsPerTeam.map(roomType => ({
			roomType,
			assignments: this.assignments().filter(assignment => assignment.slot.type === roomType),
			roomsCount: this.emptyScheduele.allRooms.filter(room => room === roomType).length
		}))
	}

	rankingMatches () {
		return this.assignments().filter(assignment => assignment.slot.stage === 'ranking')
	}

	practiceMatches () {
		return this.assignments().filter(assignment => assignment.slot.stage === 'practice')
	}

	startTime () {
		return new Date(`01/01/2000 ${this.emptyScheduele.startTime}`)
	}

	roomSpan () {
		return this.emptyScheduele.roomSpan
	}

	matchSpan () {
		return this.emptyScheduele.matchSpan
	}
